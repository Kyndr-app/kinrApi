...
router.get("/:name", async (req, res) => {
  try {
    const organization = await kyndrService.findOneOrganization({
...
router.post("/", async (req, res) => {
  try {
    const organization = await kyndrService.addOrganization(
...
router.patch("/:name", async (req, res) => {
  try {
    const organization = await kyndrService.updateOrganization(
      { name: req.params.name },
      req.body.organization
    );
